...
async def sl(ctx, *, args):
    ...
    for word in search_result:
        ...
        value = "```"
        if counter == 0:
            embed_list.add_page(embed=None, audio=[], link=[])
...
async def randomsl(ctx):
    ...
    embed_list.add_page(embed, [0], [result['taishaneseAudio']])
        ...
        if channel != voice.channel:
            await ctx.send(
                content=f"{ctx.author.mention} you need to be in the same "
                        f"channel as the bot to use this command!")
        else:
            voice.pause()
    else:
        await ctx.send(
            content=f"{ctx.author.mention} please connect to a voice channel "
                f"to use this command!", delete_after=30)
